...
class Level2Strategy: Strategy {
    
    var scoreIncrement: Int {
        return 2
    }
    
    private let minCoefficient: CGFloat = 0.4
    ...
    private func initializeFactories() {
        self.shapeFactory = CircleShapeFactory(minCoefficient: minCoefficient, maxCoefficient: maxCoefficient)
        self.shapeViewFactory = CircleShapeViewFactory(size: CGSize(width: Defaults.Widths.shapeWidth, height: Defaults.Widths.shapeWidth))
    }
